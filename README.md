...
See `CITATION.cff` for structured metadata. For the theorem-centered representation article, cite the neutral maintenance snapshot `v2.4.1`.
...
| Audit des prémisses | `W0`–`W5` auditent la netteté dimensionnelle et la nécessité relative des prémisses séparément exposées au niveau poids ; `D1`–`D3` portent les tests de suppression à l'interface d'espérance rationnelle ; L3 audite deux questions de minimalité de formulation. Il s'agit de résultats relatifs à des interfaces fixées, pas d'une indépendance axiomatique globale. |
...
Voir `CITATION.cff` pour les métadonnées structurées. Pour l'article centré sur les théorèmes de représentation, citer le snapshot de maintenance neutre `v2.4.1`.
