...
| Contre-modèle niveau poids (`n = 2`) | `grain_does_not_imply_born_at_two` | `witnessState` (3/5, 4/5), perspective binaire `witnessLine`/`witnessLineᗮ` | `skewWeight witnessState`, cohérent sous `AxGrain`/`AxNorm`/`AxPos`/`AxNul`, diffère de Born (`81/337 ≠ 9/25`) | Clos — réfute Grain+Norm+Pos+Null (poids) ⟹ Born en `n = 2` |
| Contre-modèle niveau décision (`n = 2`) | `decision_premises_do_not_imply_born_at_two` | `skewExpectationFamily : RationalExpectationFamily 2` (poids canonique = `skewWeight witnessState`, via `skewExpectationFamily_canonicalWeight_eq`) ; `RefinementInvariantLocal F.V` obtenue par `refinementInvariantLocal_iff_axGrain` | Les prémisses employées par `born_expectation_of_invariance` (famille rationnelle, invariance locale, `AxNul` du poids canonique) n'impliquent pas Born en `n = 2` ; ne bundle ni `3 ≤ n` ni `ProjectiveBornPremises` | Clos — réfute Grain+Norm+Pos+Null (décision) ⟹ Born en `n = 2` ; distinct du contre-modèle niveau poids |
| Contre-modèle niveau mesure-de-projection (`n = 2`) | `skewProjMeasure_not_representable`, `exists_nonrepresentable_projMeasure_two` | `skewProjMeasure : Gleason.ProjMeasure 2` (`μ A := skewF (‖projL A witnessState‖²)`) | `skewProjMeasure` n'est représentable par AUCUN opérateur densité au sens de `Gleason.bornValue` ; épinglage via `eq_projL_of_vanishes_on_orthogonal` force `ρ = projL (ℂ∙witnessState)`, puis contradiction exacte sur `witnessLine` (`81/337 ≠ 9/25`) | Clos — témoin ponctuel ; ne prétend pas qu'aucune `ProjMeasure 2` n'est représentable, seulement que celle-ci ne l'est pas |
...
| Weight-level countermodel (`n = 2`) | `grain_does_not_imply_born_at_two` | `witnessState` (3/5, 4/5), binary perspective `witnessLine`/`witnessLineᗮ` | `skewWeight witnessState`, coherent under `AxGrain`/`AxNorm`/`AxPos`/`AxNul`, differs from Born (`81/337 ≠ 9/25`) | Closed — refutes Grain+Norm+Pos+Null (weight) ⟹ Born at `n = 2` |
| Decision-level countermodel (`n = 2`) | `decision_premises_do_not_imply_born_at_two` | `skewExpectationFamily : RationalExpectationFamily 2` (canonical weight = `skewWeight witnessState`, via `skewExpectationFamily_canonicalWeight_eq`); `RefinementInvariantLocal F.V` obtained via `refinementInvariantLocal_iff_axGrain` | The premises used by `born_expectation_of_invariance` (rational family, local invariance, `AxNul` of the canonical weight) do not imply Born at `n = 2`; bundles neither `3 ≤ n` nor `ProjectiveBornPremises` | Closed — refutes Grain+Norm+Pos+Null (decision) ⟹ Born at `n = 2`; distinct from the weight-level countermodel |
| Projection-measure-level countermodel (`n = 2`) | `skewProjMeasure_not_representable`, `exists_nonrepresentable_projMeasure_two` | `skewProjMeasure : Gleason.ProjMeasure 2` (`μ A := skewF (‖projL A witnessState‖²)`) | `skewProjMeasure` is representable by NO density operator in the sense of `Gleason.bornValue`; pinning via `eq_projL_of_vanishes_on_orthogonal` forces `ρ = projL (ℂ∙witnessState)`, then an exact contradiction on `witnessLine` (`81/337 ≠ 9/25`) | Closed — a pointwise witness; does not claim that no `ProjMeasure 2` is representable, only that this one is not |
